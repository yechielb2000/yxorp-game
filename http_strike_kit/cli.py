...
@attacks_app.command()
def syn_flood(target: str, port: int = 80, threads: int = 500):
...
@attacks_app.command()
def brute_force(base_url: str, wordlist_path: str, workers: int = 100):
...
@attacks_app.command()
def slowloris(target: str, port: int, timeout: float = 10, workers: int = 100):
    typer.echo(
