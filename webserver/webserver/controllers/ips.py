...
from logger_setup import get_infra_logger
...
class IpsController:
    ...
    async def get_ip_info(self, address: str) -> Optional[IPAddressRead]:
        self.logger.info("Get ip info", extra={"address": address})
    ...
    async def create_ip_info(self, ip_info: IPAddressCreate) -> None:
        self.logger.info("Create ip info", extra={"ip_info": ip_info.model_dump()})
    ...
    async def upsert_ip_info(self, ip: str, country: str) -> IPAddressRead:
        self.logger.info("Upsert ip info", extra={"ip": ip, "country": country})
    ...
    async def delete_ip_info(self, address: str) -> None:
        self.logger.info("Delete ip info", extra={"address": address})
        stmt = delete(IPAddress).where(IPAddress.address == address)
        await self.postgres.execute(stmt)
        await self.postgres.commit()

    async def update_ip_info(self, ip_info: IPAddressUpdate) -> None:
        self.logger.info("Update ip info", extra={"ip_info": ip_info.model_dump()})
    ...
    async def get_top_queried_countries(self, top: int = 5) -> List[str]:
        self.logger.info("Get top queried countries", extra={"top": top})
        stmt = (
    ...
    async def get_ips_by_country(
        ...
        self.logger.info("Get ips by country",
